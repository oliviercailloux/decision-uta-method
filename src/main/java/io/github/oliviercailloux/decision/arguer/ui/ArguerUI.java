...
public class ArguerUI {
	...
	public void showNunesArgument(AlternativesComparison altsComp, Set<Constraint> constraints) {
		showHeader("Nunes");
	...
	public void showLabreucheArgument(AlternativesComparison altsComp) {
	...
	public void showCriteriaWeights(AlternativesComparison altsComp) {
		StringBuilder bld = new StringBuilder();
		
		bld.append("\n    Criteria    <-   Weight : \n");

		for (Criterion c : altsComp.getWeight().keySet()) {
			bld.append("\n" + "	" + c.getName() + "  <-  w_" + c.getId() + " = " + altsComp.getWeight().get(c));
		}
	...
	public void showAlternatives(AlternativesComparison altsComp) {
		StringBuilder bld = new StringBuilder();
				
		bld.append("\n \n Alternatives : ");

		bld.append("\n" + "	" + altsComp.getX().getName() + " " + " : " + Utils.showVector(altsComp.getX().getEvaluations().values()));
		bld.append("\n" + "	" + altsComp.getY().getName() + " " + " : "	+ Utils.showVector(altsComp.getY().getEvaluations().values()));
		
		System.out.println(bld.toString());
	}
	
	public void showAlternativesRanked(AlternativesComparison altsComp, boolean flag) {
		StringBuilder bld = new StringBuilder();
		...
		if(flag) {
			scoreX = LabreucheTools.score(altsComp.getX(), altsComp.getWeight());
			scoreY = LabreucheTools.score(altsComp.getY(), altsComp.getWeight());
		}else {		
			Table<Alternative,Alternative,Double> tradeoffs = NunesTools.computeTO(altsComp.getAlternatives(), altsComp.getWeight());
			scoreX = NunesTools.score(altsComp.getX(), altsComp.getY(), altsComp.getCriteria(), altsComp.getWeight(), tradeoffs);
			scoreY = NunesTools.score(altsComp.getY(), altsComp.getX(), altsComp.getCriteria(), altsComp.getWeight(), tradeoffs);
		}		
	...
	public void showStartExplanation(AlternativesComparison altsComp) {
		StringBuilder bld = new StringBuilder();
		
		bld.append("Explanation why " + altsComp.getX().getName() + " is better than "
